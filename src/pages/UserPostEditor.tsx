...
import { useUserPost, useCreateUserPost, useUpdateUserPost } from "@/hooks/usePosts";
...
export default function UserPostEditor() {
  ...
  const { data: existingPost } = useUserPost(isEdit ? Number(id) : undefined);
  ...
  const handleSave = () => submitWithStatus(isEdit ? undefined : 0);
  const handlePublish = () => submitWithStatus(1);
  ...
  return (
    <>
      <Helmet>
        <title>{isEdit ? "编辑文章" : "写文章"} - Blog</title>
      </Helmet>
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-foreground">{isEdit ? "编辑文章" : "写文章"}</h1>
            </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-1 space-y-6 min-w-0">
              ...
              <div>
                ...
                <Textarea
                  {...register("summary")}
                  placeholder="文章摘要（可选，留空将由 AI 自动生成）"
                  rows={2}
                  className="text-sm"
                />
              </div>
